...
Nick NVARCHAR(50) NOT NULL UNIQUE DEFAULT 'user',
Identifier INT NOT NULL DEFAULT 0,
Passphrase INT NOT NULL DEFAULT 0,
check (len(Nick)<=25),
unique (Identifier,Passphrase) 
);
create nonclustered index Authenticate on Account (Identifier, Passphrase);
...
insert into Account values ('Curiosity',2134,1234312),(N'Протон',-1245,0);
GO
CREATE PROCEDURE Authenticate 
(@LoginHash INT, @PasswordHash INT)
AS 
BEGIN
declare @Result bit;
	if ((select count(Identifier) from Account 
		where (Identifier=@LoginHash and Passphrase=@PasswordHash))>0)
			set @Result=1
		else
			set @Result=0;
   set nocount on;
...
@Email nvarchar(50)='email',@Nick nvarchar(50)='nick')
AS 
BEGIN transaction
...
(
	...
	Name NVARCHAR(50) NOT NULL default 'endpoint',
	check (len(Name)<40)
);
...
(
	Id INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
	Name NVARCHAR(50) NOT NULL UNIQUE DEFAULT 'forum',
	check (len(Name)<25)
);
...
(
	Id INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
	Name Nvarchar(100) NOT NULL UNIQUE DEFAULT 'thread',
...
END
GO
CREATE PROCEDURE GetThreadsTop9 (@EndpointId int=1)
AS 
BEGIN
	SELECT TOP 9 Id,Name FROM Thread WHERE EndpointId=@EndpointId ORDER BY Id DESC;
	set nocount on;
END
GO
...
(
	...
	MsgText NVARCHAR(1000) NOT NULL DEFAULT 'message'
);
...
create procedure PutMessage
	(@ThreadId int=1, @AccountId int=1, @Message nvarchar(1000)='message')
as
begin transaction
