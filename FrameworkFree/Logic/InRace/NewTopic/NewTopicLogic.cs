...
namespace Logic
{
    internal sealed class NewTopicLogic : INewTopicLogic
    {
        ...
        {
            SetFirstThread(Constants.One, Constants.One, GetLastThread(Constants.Zero));
        }
        ...
        private void MoveThreadsInFull(in int threadId, in int endpointId)
        {
            ...
            while (j < len)
            {
                SetFirstThread(j++, i, GetLastThread(i));
            }
        }
        private void SetFirstThread(in int next, in int current, in string lastThread)
        {
            string page = Storage.Fast.GetPage(current);
            int pos = page.IndexOf(Constants.navMarker) + Constants.navMarker.Length;
            Storage.Fast.SetPage(next, page.Insert(pos, lastThread));
        }
        ...
        private string GetLastThread(in int i)
        {
