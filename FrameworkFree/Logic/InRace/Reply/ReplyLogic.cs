...
namespace Logic
{
    internal sealed class ReplyLogic : IReplyLogic
    {
        ...
        public int? GetAccountId(in Pair pair)
        {
            ...
            {
                lock (locker)
                    accId = Sequential.Unstable.Account.GetAccIdAndStore(pair);
            }
        ...
        private void CorrectArray(in int id, in int accId, in string text)
        {
            ...
            {
                string threadName = Storage.Slow.GetThreadNameById(id);
                int sectionNum = Storage.Slow.GetSectionNumById(id);
